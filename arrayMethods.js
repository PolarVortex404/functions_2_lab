...
}, 0) //the _,0_ represents the 'intial value' which wasn't necessary here b/c we're reducing an array of numbers, but it is definitely required for reducing objects in an array
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter((element, index, wholeArray) => {
...
*/
const taxTotals = orders.map((element, index, wholeArray) => {
    return element.price * (element.tax + 1) //object.property
})
...
const bobsTotal = bobsOrders.reduce((sum, element, index, wholeArray) => {
    return sum + element.price //in order to access the value associated with
}, 0)
