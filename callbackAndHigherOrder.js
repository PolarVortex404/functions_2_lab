...
first(names, firstName => { //anonymous arrow function
...
// CODE HERE 
const contains = (arr, nameski, callback) => {
  if(arr.includes(nameski) === true) {
  callback(true)
} else {
  callback(false)
  }
...
const each = (arr, cb) => {
  arr.forEach((item, index) => cb(item, index))
}
...
// CODE HERE 
const getUserById = (arrUsers, id, callback) => { //is there a way to do this without the for loop? 
...
const addTen = addingFactory(10) //invocation = the value you're passing into the parameter as an argument
